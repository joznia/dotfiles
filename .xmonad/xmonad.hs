...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- kill xmobar
    , ((modm .|. shiftMask, xK_z     ), spawn "kill -9 $(pgrep xmobar)")
    
    -- vim
    , ((modm,               xK_v     ), spawn (myTerminal ++ " -e" ++ myEditor))

    -- file manager
    , ((modm .|. shiftMask, xK_f     ), spawn (myTerminal ++ " -e" ++ myFileManager))
...
main = do
  ...
  xmproc1 <- spawnPipe "xmobar -x 1 /home/jo/.config/xmobar/xmobarrc1"
  -- Finally, run xmonad
  xmonad $ docks def
      {
      -- simple stuff
